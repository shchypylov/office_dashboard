...
export const fetchBalance = user => async dispatch => {
  ...
  let users = await fire.database().ref("users").on("value", function (snapshot) {
    Object.keys(snapshot.val()).map(element => {
      if (user === snapshot.val()[element].login) {
         return elem.balance = snapshot.val()[element].cash
      }
    });
    return dispatch({
      type: FETCH_BALANCE,
